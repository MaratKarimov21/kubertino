...
- [ ] Context selection renders in centered bordered dialog (not full-screen)
- [ ] Dialog uses rounded border with cyan color and padding
- [ ] All keyboard navigation works (up/down/enter/esc/q)
- [ ] Auto-selection for single context still works
- [ ] Integration requirements verified
- [ ] Existing functionality regression tested (Story 2.1 behavior preserved)
- [ ] Namespace view transition works seamlessly
- [ ] Code follows existing patterns and standards
- [ ] Tests pass (existing Story 2.1 tests + any new visual tests)
- [ ] No breaking changes to `AppModel` or Update() logic
...
- `internal/tui/app.go` - Modify `renderContextList()` method (lines 995-1029)
- Potentially `internal/tui/app_test.go` - Update test expectations if needed
